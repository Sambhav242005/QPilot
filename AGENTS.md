...
1. Read `docs/SPECTS.md` for requirements and schemas.
2. Read `docs/DESIGN.md` for UI/UX specifications.
3. Read `docs/IMPLEMENTATION_PLAN.md` for phased approach.
4. Read `docs/TESTING.md` for testing strategy and conventions.
5. Read `docs/TODO.md` for current task status.
6. Read `docs/SKILL_MAP.md` for which skill to use for each task type.
...
| Database (Dev) | SQLite via SQLAlchemy 2 + aiosqlite (file-based, no server needed) |
| Database (Prod) | PostgreSQL 15+ via SQLAlchemy 2 + asyncpg |
...
- Use SQLAlchemy 2 in the FastAPI backend with Alembic migrations.
- SQLite for development (file-based, zero config).
- PostgreSQL for Docker/production (swap via `DATABASE_URL`).
- The backend must read `DATABASE_URL` for both runtime connections and migrations.
- Next.js must not access the database directly; it communicates with FastAPI over HTTP.
- Never edit migrations after they are applied.
- Use transactions for multi-step operations.
- Primary keys: UUID strings generated by the backend. Timestamps: UTC.
...
# Create a backend migration after changing SQLAlchemy models
...
8. If unsure about a requirement, check `docs/SPECTS.md` first.
